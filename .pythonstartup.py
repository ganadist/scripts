...
_PY_VER = sys.version_info.major
if _PY_VER == 2:
    reload(sys)
...
autobuiltins = ['edit', 'which', 'ls', 'll', 'lr', 'mv', 'cp', 'cpr',
        'rm', 'rmdir', 'ln', 'lnh', 'cd', 'pwd', 'pushd', 'popd', 'env', 'mkdir', 'help', ]
...
if sys.stderr.isatty():
    if TERM in ['xterm', 'vt100', 'xterm-256color']:
...
if _PY_VER == 2:
    btn = '__builtins__.__dict__["%s"] = %s'
elif _PY_VER == 3:
    btn = 'import builtins;builtins.__dict__["%s"] = %s'
else:
    pass

for n in autobuiltins:
    exec(btn % (n, n), globals())
